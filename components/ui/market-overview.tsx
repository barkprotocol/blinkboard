...
const COIN_IDS = {
  ...
  sol: 'solana',
  usdc: 'usd-coin',
  bark: 'bark' // Assuming BARK is listed on CoinGecko, replace with actual ID if different
}
...
export function BARKMarketOverview() {
  ...
  return (
    <Card className="bg-white dark:bg-gray-800 shadow-lg overflow-hidden">
      <CardHeader className="space-y-6 bg-gray-50 dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700">
        <div className="flex flex-col space-y-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
            <CardTitle className="text-xl sm:text-2xl font-semibold text-gray-800 dark:text-gray-100">{getTitle}</CardTitle>
        ...
        {isLoading ? (
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-2 sm:space-y-0 mb-6">
              ...
              <Badge variant={parseFloat(getPercentageChange) >= 0 ? "success" : "destructive"} className="flex items-center self-start sm:self-auto text-base sm:text-lg px-2 sm:px-3 py-0.5 sm:py-1">
                {parseFloat(getPercentageChange) >= 0 ? (
                  <ArrowUpIcon className="w-4 h-4 sm:w-5 sm:h-5 mr-1" />
                ) : (
                  <ArrowDownIcon className="w-4 h-4 sm:w-5 sm:h-5 mr-1" />
                )}
            ...
            <div className="h-[250px] sm:h-[300px] mb-6">
              <ResponsiveContainer width="100%" height="100%">
                <RechartsLineChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
                  ...
                  />
                  <RechartsTooltip 
                    contentStyle={{ backgroundColor: '#1F2937', border: 'none', borderRadius: '8px', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' }}
                  ...
                  <Line 
                    ...
                    strokeWidth={2}
                    dot={false}
                    activeDot={{ r: 6, fill: '#D0BFB4', stroke: '#fff' }}
                  />
                </RechartsLineChart>
              </ResponsiveContainer>
            </div>
              ...
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <InfoIcon className="w-4 h-4 sm:w-5 sm:h-5 text-gray-400 cursor-help" />
                  </TooltipTrigger>
                  <TooltipContent>
                    <p>Data sourced from CoinGecko API</p>
                    <p>Last 30 days of market data</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
